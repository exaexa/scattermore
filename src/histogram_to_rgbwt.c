                   ...
                   const float *histogram)
{
  ...
  const size_t offset_R = 0;
  const size_t offset_G = 1;
  const size_t offset_B = 2;
  const size_t offset_W = 3;
  const size_t offset_T = 4;
  ...
  for (i = 0; i < size_out; ++i) {
    ...
    RGBWT[offset + offset_G] = 0;
    RGBWT[offset + offset_B] = 0;
